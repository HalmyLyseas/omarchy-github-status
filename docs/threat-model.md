...
| Scoped read of `~/.config/omarchy/shell.json` | On a host without a live `shell.shellConfig` (`scopedHost`), `Service.qml` opens a watched, non-preloading `FileView` on this file, read-only, capped at 1 MiB. `Model.ownEntryFromConfigText()` parses it and retains only this plugin's own entry (a JSON round-tripped copy); anything else is discarded. This plugin never writes this file — the host writes it, via `updateEntryInline()`. |
